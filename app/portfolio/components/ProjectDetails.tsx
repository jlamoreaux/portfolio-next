...
const ProjectDetails: FC<Props> = ({
  ...
  return (
    <div
      className={`absolute bottom-0 left-0 p-6 w-full h-full flex-auto flex flex-col flex-end justify-end bg-gradient-to-b from-transparent from-5% to-white to-85% transform transition-all duration-300 ${
        isHovered ? "md:translate-y-0" : "md:translate-y-12"
      }`}
    >
      <div>
        <h3 className="text-xl font-display font-bold text-slate-900 mb-2">{title}</h3>
        <p className="text-base text-slate-700 mb-4 leading-relaxed">{description}</p>
      </div>
      <div
